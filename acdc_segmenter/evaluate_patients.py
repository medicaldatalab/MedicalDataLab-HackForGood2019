...
# Modified:
# Alejandro Marcos Díaz (alejandro99marcos@gmail.com)
...
def score_data(input_folder, output_folder, model_path, exp_config, do_postprocessing=False, gt_exists=True, evaluate_all=False, use_iter=None):
    ...
    evaluate_singular = evaluate_test_set
    ...
    with tf.Session() as sess:
        ...
        for folder in folder_list:
            ...
            if os.path.isdir(folder_path):
                ...
                elif evaluate_singular:
                    train_test = 'singular'
                else:
                    train_test = 'test' if (int(folder[-3:]) % 5 == 0) else 'train'


                if train_test == 'test' or train_test == 'singular':
                    ...
                    for file in glob.glob(os.path.join(folder_path, 'patient???_frame??.nii.gz')):
                        ...
                        # Save prediced mask
                        if evaluate_singular:
                            out_file_name = os.path.join(output_folder, 'patient' + patient_id + frame_suffix + '.nii.gz')
                        else:
...
def score_data_singular(input_folder, output_folder, model_path, patient_id, exp_config, do_postprocessing=False, gt_exists=True, evaluate_all=False, use_iter=None):
    ...
    evaluate_singular = not gt_exists
        ...
        for file in glob.glob(os.path.join(folder_path, 'patient???_frame??.nii.gz')):
            ...
            if exp_config.data_mode == '2D':
                ...
                for zz in range(img.shape[2]):
                    ...
                    # RESCALING ON THE LOGITS
                    # This can occasionally lead to wrong volume size, therefore if gt_exists
                    # we use the gt mask size for resizing.
                    prediction = transform.rescale(slice_predictions,
                                                   (1.0/scale_vector[0], 1.0/scale_vector[1], 1),
                                                   order=1,
            ...
            out_file_name = os.path.join(output_folder, patient_id + frame_suffix + '.nii.gz')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--evaluate_singular', action='store_true')
    parser.add_argument("PATIENT_PATH", type=str, help="Path to the patient folder")
    parser.add_argument("PATIENT_ID", type=str, help="PatientID")
    args = parser.parse_args()

    evaluate_test_set = args.evaluate_test_set
    evaluate_all = args.evaluate_all
    evaluate_singular = args.evaluate_singular
    ...
    if evaluate_test_set:
        ...
        logging.warning('EVALUATING ON SINGULAR DATA')
        input_path = patient_path
    ...
    if evaluate_singular:
        path_pred = patient_path
        
    if evaluate_singular:
        init_iteration = score_data_singular(input_path, 
                                             output_path,
                                             model_path,
                                             patient_ID,
                                             exp_config=exp_config,
                                             do_postprocessing=True,
                                             gt_exists=(evaluate_all),
